...
export default {
    listaComentarios: async (req,res)=>{
        try{
            let comentarios = await comentSchema.find().populate('usuario').populate('post')
    ...
    inserirComentario:async (req,res)=>{
        let corpo = req.body
        try{
            let comentario = await comentSchema.create(corpo).populate('usuarios').populate('posts')
    ...
    obterComentario: async (req,res)=>{
        ...
        try{
            let comentario = await comentSchema.findById(id).populate('usuarios').populate('posts')
            // comentario = view.render(comentario)
